...
public class Piece {//all other pieces are extensions of this class, contains all piece methods except for check legal move and special moves 
    ...
    public void editMoveCount(int edit){
        this.movecount = edit;
    }
    public void decrementMoveCount(){
        this.realMoveCount--;
        if(realMoveCount==0){
            this.movecount = realMoveCount;
        }
    }
    ...
    /**
     * moves the piece to the old location that it was at 
     */
    public void flipLoc(){
        int storedX = this.getX();
        int storedY = this.getY();
        setX(getX1());
        setY(getY1());
        setX1(storedX);
        setY1(storedY);
     ...
     * @param x x coordinate
     * @param y y coordinate
     * @param addMove used to make sure we are adding moves to the thing 
     * @return Returns an integer based on situation to be handled inside process move in the main method
     */

    public int checkPossibleMoves(int x, int y, boolean addMove){//
                ...
                if(possiblemovesX.get(i)==x&&possiblemovesY.get(i)==y&&checkbounds(x,y)){//if the move made is possible 
                   ...
                   boards.getBoard()[getY1()][getX1()]="00000";
                    ...
                    if(addMove){
                        ...
                        for(int j=0; j < 8; j++){
                            for(int k =0; k < 8;k++){
                                allStuff[j][k] = boards.getBoard()[j][k];
                            }
                        }
                       
                        boards.allMoves.add(allStuff);
                        boards.displayBoard();
                        System.out.println("running here" + boards.allMoves.size());
                    }
                    deleteMoves();
